...
signed-token support, a production HTTPS client `allowedOrigin` with no path,
query, hash, localhost, or private-network host, `Idempotency-Key` preflight
support, idempotency metadata, rate-limit metadata, and closed circuit-breaker
metadata. Final evidence must use an ISO
